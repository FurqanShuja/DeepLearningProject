...
from dlcv.utils import custom_collate_fn, get_transforms, plot_image_with_boxes
from dlcv.dataset2 import Dataset2
from dlcv.model import fasterrcnn_model, fcos_model, retinanet_model
from torch.utils.data import DataLoader

def evaluate_one_epoch(model, data_loader, device, save_dir, max_images, score_threshold, iou_threshold):
    model.eval()
    save_count = 0
        ...
        for batch in tqdm(data_loader, desc="Evaluating"):
            ...
            for i, output in enumerate(outputs):
                ...
                if save_count < max_images:
                    image_save_path = os.path.join(save_dir, f"{file_name}_pred.png")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Visualize the Faster R-CNN model predictions.')
